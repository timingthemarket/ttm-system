...
namespace securities_masterdata.DataAccess.Extensions;

public static class HttpClientExtensions
{
    public static async Task<T?> GetJson<T>(this HttpClient client, string path,
        NameValueCollection? queryKeyValue = null,
        JsonTypeInfo<T>? typeInfo = null, CancellationToken token = default)
    {
        if (client.BaseAddress is null)
            throw new ArgumentNullException(nameof(client.BaseAddress),
                "Please provide a BaseAddress to use this method");
        ...
        return typeInfo is null
            ...
            : JsonSerializer.Deserialize(response, typeInfo);
    }

    public static async Task<T?> GetJson<T>(this HttpClient client, Uri baseAddress, string path,
        NameValueCollection? queryKeyValue = null,
        JsonTypeInfo<T>? typeInfo = null, CancellationToken token = default)
    {
